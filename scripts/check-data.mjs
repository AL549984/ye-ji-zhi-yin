...
if (productData.events.length !== 36) throw new Error(`Expected 36 events for the workbench, got ${productData.events.length}.`);
...
for (const item of productData.events) {
  ...
  if (!/^\d{4}-\d{2}(-\d{2})?(~\d{2})?$/.test(item.date)) throw new Error(`Invalid event date for ${item.company}: ${item.date}`);
